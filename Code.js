...
function getActiveAds(){
    ...
    else if(!CAMPAIGN_CONTAINS && CAMPAIGN_DOES_NOT_CONTAIN){var query = 'Status = ENABLED AND CampaignName DOES_NOT_CONTAIN ' + parse(CAMPAIGN_DOES_NOT_CONTAIN)}
    else if(CAMPAIGN_CONTAINS && CAMPAIGN_DOES_NOT_CONTAIN){var query = 'Status = ENABLED AND CampaignName CONTAINS ' + parse(CAMPAIGN_CONTAINS) + ' AND CampaignName DOES_NOT_CONTAIN ' + parse(CAMPAIGN_DOES_NOT_CONTAIN)}
    else if(!CAMPAIGN_CONTAINS && !CAMPAIGN_DOES_NOT_CONTAIN){var query = 'Status = ENABLED'}
...
function resumeAds(ads, labelName){
    ...
    while(ads.hasNext()){
        ...
        if(isOutOfStock == -1){
            ...
            ad.enable()
            var labels = ad.labels().withCondition('LabelName = ' + labelName).get().next().remove()
        }
    }
