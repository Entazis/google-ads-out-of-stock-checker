...
function getActiveAds(){
    var query = '';
    if(CAMPAIGN_CONTAINS && !CAMPAIGN_DOES_NOT_CONTAIN){query = 'Status = ENABLED AND CampaignName CONTAINS ' + parse(CAMPAIGN_CONTAINS)}
    else if(!CAMPAIGN_CONTAINS && CAMPAIGN_DOES_NOT_CONTAIN){query = 'Status = ENABLED AND CampaignName DOES_NOT_CONTAIN ' + parse(CAMPAIGN_DOES_NOT_CONTAIN)}
    else if(CAMPAIGN_CONTAINS && CAMPAIGN_DOES_NOT_CONTAIN){query = 'Status = ENABLED AND CampaignName CONTAINS ' + parse(CAMPAIGN_CONTAINS) + ' AND CampaignName DOES_NOT_CONTAIN ' + parse(CAMPAIGN_DOES_NOT_CONTAIN)}
    else if(!CAMPAIGN_CONTAINS && !CAMPAIGN_DOES_NOT_CONTAIN){query = 'Status = ENABLED'}
...
function pauseAds(ads, labelName){
    Logger.log('Checking enabled ads... Number of ads to check: ' + ads.totalNumEntities());

    while(ads.hasNext()){
        var ad = ads.next();
        var url = ad.urls().getFinalUrl();
        var content = UrlFetchApp.fetch(url).getContentText();
        if(url.search(DETAIL_PAGE_STRING) === -1){continue}
...
function resumeAds(ads, labelName){
    Logger.log('Checking paused ads... Number of ads to check: ' + ads.totalNumEntities());
    ...
    while(ads.hasNext()){
        ...
        if(isOutOfStock === -1){
            ...
            ad.enable();
            ad.labels().withCondition('LabelName = ' + labelName).get().next().remove();
        }
    }
}
